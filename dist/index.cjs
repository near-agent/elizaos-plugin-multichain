...
var multichainTools = require('multichain-tools');
...
function parseAmount(n, decimals) {
  return BigNumber__default.default(n).shiftedBy(decimals);
}
function parseBTC(n) {
...
var DerivedAddressProvider = class {
  ...
  async getDerivedAddress(runtime) {
    try {
      ...
      const bitcoin = new multichainTools.Bitcoin(getBitcoinConfig(runtime));
      const { address: btcAddress } = await bitcoin.deriveAddressAndPublicKey(this.accountId, getDerivationPath("BTC"));
      const evm = new multichainTools.EVM(getEvmConfig(runtime));
      const { address: evmAddress } = await evm.deriveAddressAndPublicKey(this.accountId, getDerivationPath("EVM"));
      const addresses = {
        btc: btcAddress,
        evm: evmAddress
      };
      core.elizaLogger.info(`Chain Signatures derived addresses:`, addresses);
      this.cache.set(cacheKey, addresses);
      return addresses;
    } catch (error) {
      core.elizaLogger.error(`Error in derived address provider: ${error}`);
      return null;
    }
  }
};
var walletProvider = {
  get: async (runtime, _message, _state) => {
    try {
      ...
      core.elizaLogger.error(`Error in get derived address provider: ${error}`);
      return null;
    }
  }
};
...
async function transferBTC(runtime, recipient, amount) {
  ...
  const bitcoin = new multichainTools.Bitcoin(config);
  ...
  const response = await multichainTools.signAndSend.keyPair.signAndSendBTCTransaction({
    transaction: {
      to: recipient,
    ...
    derivationPath
  }, nearApiJs.KeyPair.fromString(secretKey));
  if (response.success) {
...
async function transferEth(runtime, recipient, amount) {
  ...
  const evm = new multichainTools.EVM(config);
  const derivationPath = getDerivationPath("EVM");
  const { address } = await evm.deriveAddressAndPublicKey(accountId, derivationPath);
  const response = await multichainTools.signAndSend.keyPair.signAndSendEVMTransaction({
    transaction: {
      to: recipient,
...
var executeTransfer = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    if (!isTransferContent(runtime, content)) {
      core.elizaLogger.error("Invalid content for MULTI_CHAIN_TRANSFER_TOKEN action:", content);
      if (callback) {
        callback({
...
Transaction: ${txHash}`,
      ...
      core.elizaLogger.error(`Error during ${content.symbol} transfer: ${error}`);
      if (callback) {
        callback({
...
exports.multichainPlugin = multichainPlugin;
